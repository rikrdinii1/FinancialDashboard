...
def plot_data(data, indicators, sync_axis=None):
    ...
    p.vbar(df.index[gain], width, df.Open[gain], df.Close[gain],
           fill_color='#00f00', line_color='#00ff00')
    ...
    for indicator in indicators:
        if indicator == '30 Day SMA':
            ...
            p.line(df.index, df.SMA30, color = 'purple', legend_label = '100 Day SMA')
        elif indicator == 'Linear Regression Line':
            par = np.polifit(range(len(df.index.values)), df.Close.values, 1, full = True)
            slope = par[0][1]
            intercept = par[0][1]
            y_pred = [slope * i + intercept for i in range(len(df.index.values))]
            p.segment(df.index[0], y_pred[0], df.index[-1], y_pred[-1], legend_label = "Linear Regresion", color = 'red')
...
layout = column(stock1_text, stock2_text, date_picker_from,
                date_picker_to, indicator_choice, load_button)
