...
def plot_data(data, indicators, sync_axis = None):
    ...
    if sync_axis is not None:
        p = figure(x_axis_type = 'datetime', tools = 'pan, wheel_zoom, box_zoom, reset, save', width = 1000, x_range = sinc_axis)
    else:
    ...
    p.vbar(df.index[gain], width, df.Open[gain], df.Close[gain], fill_color = '#00f00', line_color = '#00ff00')
